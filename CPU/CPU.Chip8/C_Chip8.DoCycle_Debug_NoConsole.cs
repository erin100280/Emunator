	...
	public partial class C_Chip8 : C_Base {
		public override bool DoCycle_Debug_NoConsole() {
			...
			ushort oc=m_opcode=(ushort)(m_bank[m_counter] << 8
						| m_bank[m_counter+1]);
			ushort ushH, ushW, x, y, h, pxl, I = m_indexRegister;
			string s;
			#endregion
			#region DBG
			#if(DBG)
   			...
   			#region 0x00E.  0x00E0, 0x00EE
				case 0x00E0:
	   			...
	   			case 0x000E:
						...
						WriteDoCycle("0x00EE"
		            ,	"return from sub to: "
		            +	m_stack[m_stackCount - 1].ToString("X")
		            +	" / " + m_stack[m_stackCount - 1]
						);
						#endif
						#endregion
	   				if(m_stackCount > 0) {
			...
			case 0xF000:
				...
				case 0x001E:
					#region DBG
						...
						+	" overflow=" + i
						+	" val="
						+	(m_indexRegister + regs[(oc & 0x0F00) >> 8]).ToString("X")
...
//*/
				...
				case 0x0065:
					#region DBG
						...
						+	" to addr (I+X["
						+		(m_indexRegister + ((oc & 0x0F00) >> 8)).ToString("X")
						+		" / " + (m_indexRegister + ((oc & 0x0F00) >> 8))
						+	"]"
						);
					#endif
					#endregion
