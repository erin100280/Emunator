...
namespace Emu.Core {
	...
	public class ulongArrayDict : Dictionary<string, ulong[]> {}
	public class ushortArrayDict : Dictionary<string, ushort[]> {}
	...
	public class state {
		...
		public ushortArrayDict ushortArrays;
		...
		protected virtual void InitState() {
			...
			ushortArrays = new ushortArrayDict();
	...
	public static class stateSystem {
		public static void SaveState(state val, string filename) {
			try {
				...
				#endregion
				#region ushortArrays
/*				bw.Write(val.ushortArrays.Count); //-write dict count (4 bytes)
				//- write values
				foreach(KeyValuePair<string, ushort[]> v in val.ushortArrays) {
		         bw.Write(v.Key); //-write key
		         usArr = v.Value;
		         if(usArr == null) //-if array is null, create empty one
		         	usArr = new ushort[0];
		         bw.Write(usArr.Length); //-write arr len (4b)
		         for(int i = 0, il = usArr.Length; i < il; i++)
		         	bw.Write(usArr[i]); //-write arr
				}
//*/
				#endregion
		...
		public static state LoadState(string filename) {
			...
			try {
				...
				#region ushortArrays
				//for(i = 0, il = br.ReadInt32(); i < il; i++)
				//	rv.ushortArrays.Add(br.ReadString(), br.reReadBytes(br.ReadInt32()));
				#endregion
