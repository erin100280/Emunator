...
namespace Emu.Device.Input.Keyboard {
	...
	/// </summary>
	#endregion
	public class Keyboard_Base : iInput {
		#region vars
		...
		#endregion
		#region On....
		public virtual bool KeyDownInput(KeyEventArgs e) {
			if(_keyBuffer != null && _keyMap != null) {
				for(Int32 ii = 0, il = _keyMap._keys.Length; ii < il; ii++) {
					if(_keyMap._keys[ii].mapKeyboard == e.KeyValue) {
						_keyBuffer[_keyMap._keys[ii].keyValue] = 0x01;
						return true;
					}
				}
			}
		...
		public virtual bool KeyUpInput(KeyEventArgs e) {
			if(_keyBuffer != null && _keyMap != null) {
				for(Int32 ii = 0, il = _keyMap._keys.Length; ii < il; ii++) {
					if(_keyMap._keys[ii].mapKeyboard == e.KeyValue) {
						_keyBuffer[_keyMap._keys[ii].keyValue] = 0x00;
						return true;
					}
				}
			}
		...
		public virtual bool OnKeyPup(KeyEventArgs e) {
			bool rv = false;
